 ...
 * Fetches data from the GitHub API and displays the repository name, last push date, and a link to the repository.
 */
const GitReposComponent: FunctionComponent = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            console.error('This is wrong', error)
        }
    }
    // Fetches the latest repositories from the GitHub API on component mount.
    useEffect(() => {
        fetchData()
    }, [])
            ...
            <div className="grid grid-flow-row md:grid-flow-col pt-4">
                {isLoading ? (
                    <div className="flex gap-3">
                        ...
                        <div>
                            <p className="text-red-400">Still loading for some reason..</p>
                        </div>
                    </div>
                ) : (
                    <>
